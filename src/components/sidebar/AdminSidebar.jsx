...
import { 
  ...
  Building2, 
  User, 
  Users, // <-- Imported Users icon for the new menu item
  Sparkles,
  X
} from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  // ==========================================
  useEffect(() => {
    // This listens to the event fired by AdminNavbar.jsx
  ...
  const navItems = [
    { id: 'Overview', icon: LayoutGrid, path: '/admin/dashboard' },
    { id: 'Approvals', icon: ShieldCheck, path: '/admin/approvals' },
    { id: 'Calendar', icon: CalendarDays, path: '/admin/calendar' },
    { id: 'Halls', icon: Building2, path: '/admin/halls' },
    { id: 'Users', icon: Users, path: '/admin/users' }, // <-- Added Users route here
    { id: 'Profile', icon: User, path: '/admin/profile' }, 
  ];
              ...
              <div>
                <div className="flex items-center justify-between px-6 mb-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h1 className="text-[20px] font-bold text-gray-900 leading-none">Aurora</h1>
                      <p className="text-[10px] font-semibold text-[#8C7CFF] tracking-wider uppercase mt-0.5">Admin Workspace</p>
                    </div>
                  </div>
                ...
                <nav className="flex flex-col gap-1.5 px-4">
                  {navItems.map((item) => {
                    ...
                    return (
                      <NavLink key={item.id} to={item.path} className={({ isActive }) => `flex items-center px-4 py-3.5 rounded-[1.25rem] transition-all duration-200 ${isActive ? 'bg-[#8B7AFF]/10 text-[#8B7AFF] font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
        ...
        <div>
            ...
            <AnimatePresence>
              {isOpen && (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }} className="ml-3 whitespace-nowrap overflow-hidden">
                  ...
                  <p className="text-[10px] font-bold text-[#8C7CFF] tracking-wider uppercase">Admin Workspace</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          <nav className="flex flex-col gap-1.5 px-4">
            {navItems.map((item) => {
              const Icon = item.icon;
              return (
                <NavLink key={item.id} to={item.path} className={({ isActive }) => `flex items-center px-3 py-3.5 rounded-[1.25rem] transition-all duration-300 relative overflow-hidden ${isActive ? 'bg-gradient-to-r from-[#8B7AFF] to-[#9B88FF] text-white shadow-[0_4px_15px_rgba(139,122,255,0.25)]' : 'text-gray-600 hover:bg-[#E5E9F5] hover:text-gray-900'}`}>
                  {({ isActive }) => (
                    <>
