...
import { MapPin, Calendar, Users, Loader2, Check, X, RefreshCw, ScanLine, XCircle, AlertCircle, Activity } from 'lucide-react';
...
export default function Approvals() {
  ...
  const fetchApprovalsData = async () => {
    ...
    try {
      ...
      toast.error("Failed to sync latest approvals.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // NATIVE CAMERA SUPPORT: Check if Admin scanned via native phone camera
  useEffect(() => {
      ...
      try {
        ...
        if (extractedScanId) {
          ...
          if (found) {
            ...
            setScannedBooking(found); // Open details modal
  ...
  const handleStatusUpdate = async (bookingId, newStatus) => {
    ...
    try {
      ...
      // If updating from the modal, update the modal's state too
  ...
  const getStatusBadge = (status) => {
    ...
    if (status === 'Rejected') return 'bg-red-50 text-red-500 border border-red-100';
    if (status === 'Under Review') return 'bg-blue-50 text-blue-500 border border-blue-100';
    return 'bg-amber-50 text-amber-600 border border-amber-100'; // Submitted / Pending
  };

  const getStatusDot = (status) => {
    if (status === 'Approved') return 'bg-emerald-500';
    if (status === 'Rejected') return 'bg-red-500';
    if (status === 'Under Review') return 'bg-blue-500 animate-pulse';
    return 'bg-amber-500 animate-pulse';
  };

  const containerVariants = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.1 } } };
  const itemVariants = { hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 300, damping: 24 } }, exit: { opacity: 0, scale: 0.95, transition: { duration: 0.2 } } };
      ...
      <div className="max-w-[1400px] w-full mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="flex items-start gap-4">
            <div>
              <h1 className="text-3xl md:text-[2.5rem] font-bold text-gray-900 tracking-tight mb-2">Approvals</h1>
              <p className="text-gray-500 text-sm md:text-[15px] font-medium">Review booking requests and notify users.</p>
            </div>
          ...
          {/* Filter Tabs - Updated to match new schema statuses */}
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 overflow-x-auto hide-scrollbar pb-2 lg:pb-0">
            {['Submitted', 'Under Review', 'Approved', 'Rejected', 'All'].map((filter) => (
              <button
                key={filter}
                onClick={() => setActiveFilter(filter)}
                className={`px-6 py-2.5 rounded-2xl text-[13px] font-bold transition-all whitespace-nowrap active:scale-95 ${
                  activeFilter === filter ? 'bg-[#8C7CFF] text-white shadow-[0_4px_15px_rgba(140,124,255,0.3)]' : 'bg-white/60 backdrop-blur-md text-gray-600 border border-white hover:bg-white hover:shadow-sm'
                }`}
              >
                {filter}
              </button>
        ...
        {/* ================= DATA LOADING OR LIST CONTAINER ================= */}
        {isLoading && bookings.length === 0 ? (
          <div className="flex items-center justify-center py-20 opacity-70">
            <Loader2 className="animate-spin text-[#8C7CFF]" size={48} />
          </div>
        ) : (
                ...
                <motion.div key={item._id} variants={itemVariants} initial="hidden" animate="show" exit="exit" layout className="bg-white/80 backdrop-blur-xl border border-white rounded-[1.5rem] p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all group">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 w-full md:w-auto">
                    ...
                    <div className="flex flex-col justify-center">
                      <h3 className="text-lg font-bold text-gray-900 mb-0.5 leading-tight group-hover:text-[#8C7CFF] transition-colors">
                        {item.title} <span className="text-gray-400 text-sm ml-2 font-medium">({item.bookingId || item._id.substring(0, 6).toUpperCase()})</span>
                      </h3>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between md:justify-end w-full md:w-auto gap-4 md:gap-3 pt-4 md:pt-0 border-t border-gray-100 md:border-0 shrink-0">
                    <div className={`flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold shadow-sm w-full sm:w-auto ${getStatusBadge(item.status)}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${getStatusDot(item.status)}`} />
                      {item.status}
                    </div>

                    {/* FIX: Show buttons if status is Submitted OR Under Review */}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/90 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-md bg-white rounded-[2rem] overflow-hidden shadow-2xl relative">
              ...
              <div className="p-4 flex items-center justify-between border-b border-gray-100 bg-white relative z-10">
                <h3 className="font-bold text-gray-900 flex items-center gap-2"><ScanLine size={18} className="text-[#8C7CFF]" /> Scan Ticket QR</h3>
              ...
              <div className="bg-black aspect-square w-full relative overflow-hidden flex items-center justify-center">
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                  <div className="w-64 h-64 border-2 border-white/50 rounded-3xl relative">
                    <div className="absolute top-0 left-0 w-8 h-8 border-t-4 border-l-4 border-[#8C7CFF] rounded-tl-3xl -translate-x-[2px] -translate-y-[2px]" />
                    <div className="absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 border-[#8C7CFF] rounded-tr-3xl translate-x-[2px] -translate-y-[2px]" />
                    <div className="absolute bottom-0 left-0 w-8 h-8 border-b-4 border-l-4 border-[#8C7CFF] rounded-bl-3xl -translate-x-[2px] translate-y-[2px]" />
                    <div className="absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 border-[#8C7CFF] rounded-br-3xl translate-x-[2px] translate-y-[2px]" />
        ...
        {scannedBooking && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="w-full max-w-[500px] bg-white rounded-[2rem] overflow-hidden shadow-2xl">
              ...
              <div className="p-6 bg-gradient-to-br from-[#8C7CFF] to-[#6C5CE7] text-white relative">
                ...
                <p className="text-white/80 font-medium text-sm">Ticket ID: {scannedBooking.bookingId || scannedBooking._id.substring(0,8).toUpperCase()}</p>
              </div>

              <div className="p-8 space-y-4">
                <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                  <span className="text-sm text-gray-500 font-medium">Status</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusBadge(scannedBooking.status)}`}>
                ...
                {/* FIX: Only show action buttons if the booking is Submitted or Under Review */}
                {['Submitted', 'Under Review'].includes(scannedBooking.status) ? (
